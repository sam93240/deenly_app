...
def add_translations():
    ...
    # Track hadith number by counting HadithModel( occurrences
    hadith_num = 0
    i = 0
    added_expl = 0
    added_app = 0
    ...
    while i < len(lines):
        # Count hadiths
        if 'HadithModel(' in lines[i]:
            hadith_num += 1

        # Only process hadiths 8-50
        if hadith_num >= 8 and hadith_num <= 50 and hadith_num in translations:
            ...
            # Look for explication line
            if "explication: '" in lines[i] and "explicationEn:" not in lines[i]:
                # Check if next line is NOT explicationEn, insert if needed
                if i + 1 < len(lines) and "explicationEn:" not in lines[i + 1]:
                    indent = "    "
                    new_line = f'{indent}explicationEn: \'{expl_en}\',\n'
                    lines.insert(i + 1, new_line)
                    added_expl += 1
                    i += 1

            # Look for applicationQuotidienne line
            if "applicationQuotidienne: '" in lines[i] and "applicationQuotidienneEn:" not in lines[i]:
                # Check if next line is NOT applicationQuotidienneEn
                if i + 1 < len(lines) and "applicationQuotidienneEn:" not in lines[i + 1]:
                    indent = "    "
                    new_line = f'{indent}applicationQuotidienneEn: \'{app_en}\',\n'
                    lines.insert(i + 1, new_line)
                    added_app += 1
                    i += 1
    ...
    print(f"Added {added_expl} explicationEn entries")
    print(f"Added {added_app} applicationQuotidienneEn entries")
